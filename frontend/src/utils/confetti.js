...
!(function t(e, a, n, r) {
  ...
  function O(t) {
    ...
    return e.length < 6 && (e = e[0] + e[0] + e[1] + e[1] + e[2] + e[2]), { r: P(e.substring(0, 2)), g: P(e.substring(2, 4)), b: P(e.substring(4, 6)) };
  }
  function k(t) {
  ...
  function F(t, e) {
    ...
    if (((t.fillStyle = "rgba(" + e.color.r + ", " + e.color.g + ", " + e.color.b + ", " + (1 - a) + ")"), t.beginPath(), i && "path" === e.shape.type && "string" == typeof e.shape.path && Array.isArray(e.shape.matrix)))
      t.fill(
        ...
        })(e.shape.path, e.shape.matrix, e.x, e.y, 0.1 * Math.abs(o - n), 0.1 * Math.abs(l - r), (Math.PI / 10) * e.wobble)
      );
      ...
      t.ellipse
        ? t.ellipse(e.x, e.y, Math.abs(o - n) * e.ovalScalar, Math.abs(l - r) * e.ovalScalar, (Math.PI / 10) * e.wobble, 0, 2 * Math.PI)
      ...
      for (var g = (Math.PI / 2) * 3, p = 4 * e.scalar, b = 8 * e.scalar, M = e.x, v = e.y, y = 5, x = Math.PI / y; y--; )
        (M = e.x + Math.cos(g) * b), (v = e.y + Math.sin(g) * b), t.lineTo(M, v), (g += x), (M = e.x + Math.cos(g) * p), (v = e.y + Math.sin(g) * p), t.lineTo(M, v), (g += x);
  ...
  function A(t, a) {
    ...
    function p(e, a, o) {
      for (
        ...
        R.push(
          ((l = { x: N, y: z, angle: g, spread: p, startVelocity: b, color: C[A % C.length], shape: P[((f = 0), (u = P.length), Math.floor(Math.random() * (u - f)) + f)], ticks: I, decay: M, gravity: v, drift: y, scalar: O, flat: k }),
          (s = void 0),
          (h = void 0),
  ...
  (a.exports = function () {
    ...
    (a.exports.shapeFromPath = function (t) {
      ...
      if (!a) {
        for (var o, l, s = 1e3, c = s, h = s, f = 0, u = 0, d = 0; d < s; d += 2) for (var m = 0; m < s; m += 2) r.isPointInPath(n, d, m, "nonzero") && ((c = Math.min(c, d)), (h = Math.min(h, m)), (f = Math.max(f, d)), (u = Math.max(u, m)));
        (o = f - c), (l = u - h);
